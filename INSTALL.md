...
运行结果的第一条结果消息必须先给一个紧凑选择块：每条只占两行，第一行是保留原始题材的真实建议标题，第二行是实际的副标题/业务简介，末尾统一写“请回复：1,2”。用户回复编号后才继续深读或生成飞书写入预览；选择块之后再附标题机制、原始证据、深读建议、覆盖边界和短期预检结果。没有原生多选控件时使用编号回复，不伪装弹窗。短期重复执行先对各列表前 3 屏做稳定 ID 预检；本次边界没有新增时只报告覆盖边界并停止后续理解，不重复打开历史内容。只有列表层证据时，也要输出标记为 `provisional_list_evidence` 的暂定选题；完成平台原生理解后再升级为正式候选。小红书至少补充封面图与标题关系，抖音至少补充开场画面、屏幕文字及可见口播/字幕状态。酒吧、KTV、亲子、AI、消费等题材不得被强行改写成社群标题。公开数据按同平台同指标百分位进入工作权重，不直接比较跨平台绝对值。辅助脚本不会自动选 TOP。
