...
运行结果的主报告必须先给保留原始题材与核心切入点的“建议标题”：先回答内容本身值得讲什么，再单独说明能否转入门店社群。只有列表层证据时，也要输出标记为 `provisional_list_evidence` 的暂定选题和后续深读队列；完成平台原生理解后再升级为正式候选。小红书至少补充封面图与标题关系，抖音至少补充开场画面、屏幕文字及可见口播/字幕状态。酒吧、KTV、亲子、AI、消费等题材不得被强行改写成社群标题。公开数据按同平台同指标百分位进入工作权重，不直接比较跨平台绝对值。随后附注意力机制、业务关联/适配切口、平台原生证据、表现权重和下一步验证；辅助脚本不会自动选 TOP。
